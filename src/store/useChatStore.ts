    ...
    export const useChatStore = create<ChatState>((set: (fn: (state: ChatState) => ChatState) => void, get) => ({
        ...
        createThread: async (initialMessage: string) => {
            ...
            try {
                ...
                try {
                    await apiService.executeCalendarCommand(initialMessage);
                } catch (error: any) {
        ...
        sendMessage: async (content: string, threadId: string) => {
            ...
            try {
                ...
                let calendarError;
                try {
                    await apiService.executeCalendarCommand(content);
                } catch (error: any) {
                    if (error.type === 'CALENDAR_CONFLICT') {
                        calendarError = { type: 'CALENDAR_CONFLICT', ...error };
                    } else {
                        throw error;
                    }
                }
                ...
                if (calendarError) {
                    const suggestion = calendarError.suggestion ? new Date(calendarError.suggestion).toLocaleString() : 'no alternative found';

                    botResponse = `There's a scheduling conflict. ${calendarError.error}. Would you like to schedule it for ${suggestion} instead?`;
                }
